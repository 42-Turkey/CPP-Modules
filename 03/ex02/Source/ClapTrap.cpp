...
ClapTrap::ClapTrap ( std::string name_ ) : _name ( name_ ), _hitPoints ( 100 ), _energyPoints ( 100 ), _attackDamage ( 30 ){
...
void	ClapTrap::attack ( const std::string &target ) {
	( (int)_energyPoints > 0 && (int)_hitPoints > 0 ) ? --_energyPoints, std::cout << getGreen() << " ClapTrap [" << _name << "]" << getReset() << "attacks "
...
void	ClapTrap::takeDamage( unsigned int amount_ ) {
	if ( (int)( _hitPoints - amount_ ) <= 0 ) {
		_hitPoints = 0;
		std::cout << getGreen() << "[" << _name << "]" << " died."<< getReset() << std::endl;
	} else {
		_hitPoints -= amount_;
